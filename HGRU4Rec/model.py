...
class HGRU4Rec:
  ...
  def __init__(self, sess, session_layers, user_layers, n_epochs=10, batch_size=50, learning_rate=0.001,
               decay=0.96, grad_cap=0, sigma=0, dropout_p_hidden_usr=1.0,
               dropout_p_hidden_ses=1.0, dropout_p_init=0.0, init_as_normal=False,
               ...
               user_to_output=False, user_to_session_act='tanh', n_items=4, checkpoint_dir='', log_dir='', is_training=True, test_metrics="recall"):
    ...
    if ckpt and ckpt.model_checkpoint_path:
      # self.saver.restore(self.sess, '{}/hgru-model-{}'.format(self.checkpoint_dir, 100))
  ...
  ###########################METRICS###############################
  def recall(self, yhat, y, topk=5):
    rank, ids = tf.math.top_k(yhat, k=topk, sorted=True, name=None)
    y = tf.expand_dims(tf.cast(y, tf.int32), 1)
    hit = tf.math.reduce_any(tf.math.equal(ids, y), axis=1, keepdims=False, name=None)
    return tf.reduce_mean(tf.cast(hit, tf.float32))
  
  def MRR(self, yhat, y, topk=5):
    rank, ids = tf.math.top_k(yhat, k=topk, sorted=True, name=None)
    y = tf.expand_dims(tf.cast(y, tf.int32), 1)
    mask = tf.math.equal(ids, y)
    hit = tf.math.reduce_any(mask, axis=1, keepdims=False, name=None)

    cast_mask = tf.cast(mask, tf.float32)
    ranks = tf.argmax(cast_mask, axis=1)
    mrr = tf.cast(1/(1+ranks), tf.float32)

    mrr = mrr * tf.cast(hit, tf.float32)
  ...
  def build_model(self):
    ...
    with tf.variable_scope('session_gru'):
      ...
      self.embedding = tf.get_variable('embedding', [self.n_items, self.session_layers[0]], initializer=initializer, trainable=False)    
      ...
      inputs = tf.nn.embedding_lookup(self.embedding, self.X, name='embedding_x')
      ...
      if self.is_training:
        ...
        # self.yhat = self.final_activation(logits)
        # self.cost = self.loss_function(self.Y, self.yhat)
        # tf.summary.scalar('cost', self.cost)
      else:
        logits = tf.matmul(output, softmax_W, transpose_b=True) + softmax_b
        self.yhat = self.final_activation(logits)
        # print(tf.math.argmax(self.yhat, axis=1) == self.Y)
        # self.cost = tf.reduce_mean(tf.cast(tf.math.equal(tf.cast(tf.math.argmax(self.yhat, axis=1), tf.int32), self.Y), tf.float32))
        if self.test_metrics == "recall":
          self.cost = self.recall(self.yhat, self.Y, 5)
        elif self.test_metrics == "mrr":
          self.cost = self.recall(self.yhat, self.Y, 5)
  ...
  def iterate(self, data, offset_sessions, user_indptr, reset_state=True, is_validation=False):
    ...
    while not finished:
      ...
      for i in range(session_minlen - 1):
        ...
        if self.use_embedding:
          in_session_embed = np.concatenate(data.features.values[session_start + i]).reshape([self.batch_size, self.session_layers[0]])
          
          # print(in_session_embed)
          for j in range(len(self.Hs)):
            feed_dict[self.Hs[j]] = (Hs_new[j] * 0.3 + 0.7 * in_session_embed) 
        ...
        if not self.is_training:
          # print(fetches)
          # return 1
          summary, cost, Hs_new, Hu_new  = self.sess.run(fetches, feed_dict)
          # print(cost)
          # return
        else:
          summary, cost, Hs_new, Hu_new, step, lr, _ = self.sess.run(fetches, feed_dict)
          # Hs_new -= in_session_embed
  ...
  def fit(self, train_data, valid_data=None, test_data=None, patience=3, margin=1.003, is_training = True, use_embedding = False):
    """
    :param train_data:
    :param valid_data:
    :return:
    ...
    if test_data is None:
      ...
      itemids = np.concatenate([train_data[self.item_key].unique(), test_data[self.item_key].unique()])
      itemids = np.unique(itemids)
    self.itemidmap = pd.Series(data=np.arange(self.n_items), index=itemids)

    self.use_embedding = use_embedding
    if use_embedding:
      f = open("/home/ren/Documents/6240/project/HGRU4Rec/item_embeddings.json", 'r')
      pre_embedding = json.load(f)
      pre_embed = np.zeros([self.n_items, 172])
      for item_id in pre_embedding:
        if int(item_id) in itemids:
          i = self.itemidmap[int(item_id)]
          pre_embed[i] = pre_embedding[item_id]
          # print(len(pre_embedding[item_id]))
      tf.assign(self.embedding, pre_embed)
    #   # self.session_embedding_train = train_data['features']
    #   # self.session_embedding_valid = valid_data['features']
    #   self.use_embedding = True
    #   # print(self.pre_embedding)
    #   # return
    ...
    while epoch < self.n_epochs and my_patience > 0 and my_patience > 0:
      train_cost = self.iterate(train_data, offset_sessions, user_indptr, is_validation=not is_training)
      # if np.isnan(train_cost):
      #   print('Epoch {}: Nan error!'.format(epoch, train_cost))
      #   return
      # print("Begin Validation")
      if valid_data is not None:
        valid_cost = self.iterate(valid_data, offset_sessions_valid, user_indptr_valid, is_validation=not is_training)
