...
for i in range(len(interactions)):
    interactions.at[i, 'features'] = np.fromstring(interactions.iloc[i] ['features'], sep=',')
    # print(interactions.iloc[i] ['features'].shape)
    # sys.exit(0)
...
# good_users = sess_per_user[(sess_per_user >= 2) & (sess_per_user < 200)].index
good_users = sess_per_user[(sess_per_user >= 10) & (sess_per_user < 200)].index
...
if "test" in  args.out_name:
    print('Write to disk')
    # write to disk
    subprocess.call(['mkdir', '-p', 'data/prepared/'])
    inter_dense.to_hdf(f'data/prepared/{args.out_name}.hdf','test')

elif "full_train" in args.out_name:
    ...
    # write to disk
    subprocess.call(['mkdir', '-p', 'data/prepared/'])
    inter_dense.to_hdf(f'data/prepared/{args.out_name}.hdf','train')
    test_sessions.to_hdf(f'data/prepared/{args.out_name}.hdf','test')
    train_valid_sessions.to_hdf(f'data/prepared/{args.out_name}.hdf', 'valid_train')
    valid_sessions.to_hdf(f'data/prepared/{args.out_name}.hdf','valid_test')
    ...
    # write to disk
    subprocess.call(['mkdir', '-p', 'data/prepared/'])
    train_full_sessions.to_hdf(f'data/prepared/{args.out_name}.hdf','train')
    test_sessions.to_hdf(f'data/prepared/{args.out_name}.hdf','test')
    train_valid_sessions.to_hdf(f'data/prepared/{args.out_name}.hdf', 'valid_train')
    valid_sessions.to_hdf(f'data/prepared/{args.out_name}.hdf','valid_test')
