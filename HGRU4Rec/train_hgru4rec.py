...
parser.add_argument('--dropout_p_hidden_usr', type=float, default=1.0)
parser.add_argument('--dropout_p_hidden_ses', type=float, default=0.9)
parser.add_argument('--dropout_p_init', type=float, default=.2)
...
parser.add_argument('--test_hdf_path', type=str, default=None)
args = parser.parse_args()

sessions_path = args.hdf_path
test_path = args.test_hdf_path
logger.info('Loading data from: {}'.format(sessions_path))
train_data = pd.read_hdf(sessions_path, 'train')
test_data = pd.read_hdf(test_path, 'test')
valid_data = pd.read_hdf(sessions_path, 'test') if args.early_stopping else None
